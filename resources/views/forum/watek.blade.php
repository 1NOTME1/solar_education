...
@section('content')
                ...
                @foreach ($posty as $post)
                    <div class="post-container" data-date="{{ $post->data_publikacji }}">
                                ...
                                @if ($post->uzytkownik_id == Auth::id())
                                    <div class="options">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <form method="POST" action="{{ route('forum.post.delete', $post->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="delete-post-button">Usuń</button>
                                            </form>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="post-content">
                            <p id="post-content-{{ $post->id }}">{{ $post->tresc }}</p>
                            <form method="POST" action="{{ route('forum.post.edit', $post->id) }}" class="edit-post-form"
                                id="edit-form-{{ $post->id }}" style="display: none;">
                                @csrf
                                <div>
                                    <textarea id="edit-tresc-{{ $post->id }}" name="tresc" required>{{ $post->tresc }}</textarea>
                                </div>
                                <button type="submit">Zapisz</button>
                                <button type="button" onclick="cancelEditPost({{ $post->id }})">Anuluj</button>
                            </form>
                        </div>
                        ...
                        @if ($post->komentarze && $post->komentarze->isNotEmpty())
                            @foreach ($post->komentarze as $komentarz)
                                <div class="comment-container">
                                        ...
                                        @if ($komentarz->uzytkownik_id == Auth::id())
                                            <div class="options">
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <form method="POST"
                                                        action="{{ route('forum.komentarz.delete', $komentarz->id) }}">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <form method="POST" action="{{ route('forum.komentarz.edit', $komentarz->id) }}"
                                        class="edit-comment-form" id="edit-form-{{ $komentarz->id }}"
                                        style="display: none;">
                                        @csrf
                                        <div>
    ...
    <style>
        ...
        .post-header .options:hover .dropdown {
            display: block;
        }

        .post-header .options .dropdown button {
            display: block;
            width: 100%;
            padding: 10px;
            text-align: left;
        ...
        .post-header .options .dropdown button:hover {
        ...
        body.dark-mode .new-post-form label {
            color: #ccc;
        }
        ...
        body.dark-mode .new-post-form button,
        .delete-comment-button,
        .edit-comment-button {
        ...
        body.dark-mode .new-post-form button:hover,
        .delete-comment-button:hover,
        .edit-comment-button:hover {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterPosts() {
                ...
                posts.forEach(post => {
                    ...
                    if ((!isNaN(dateFrom) && postDate < dateFrom) || (!isNaN(dateTo) && postDate >
                        dateTo)) {
